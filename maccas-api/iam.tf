...
resource "aws_iam_role" "ecs-agent" {
  name               = "maccas-refresh-worker-ecs-agent"
  assume_role_policy = data.aws_iam_policy_document.ecs-agent.json
}

resource "aws_iam_role_policy_attachment" "ecs-agent-service" {
  role       = aws_iam_role.ecs-agent.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEC2ContainerServiceforEC2Role"
}

resource "aws_iam_role_policy_attachment" "ecs-agent-resource-access" {
  role       = aws_iam_role.ecs-agent.name
...
resource "aws_iam_role" "ecs-task-execution-role" {
  name               = "maccas-refresh-worker-ecs-task-execution-role"
